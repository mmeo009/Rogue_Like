...
{
    public CharacterController controller;                  // Rigidbody 를 사용하지 않은 충돌체 캐릭터 컨트롤 용
    ...
    private void Update()
    {
        bool isGrounded = controller.isGrounded;                // CharactorController에서 땅에 있는지 판별 해준다.
    ...
    void JumpLogic(bool isGrounded)                                     // 점프 함수
    {
    ...
    void EndAction()                        // 액션 종료 시 남아있는 것들을 다 초기화
    {
        isAction = false;
        if(currentFx != null)
        {
